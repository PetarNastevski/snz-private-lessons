...
Да се направи модел за класификација за даденото податочно множество. За тренинг да се земат само првите 25 примероци од класите Bream и Perch (Првите 25 од Bream и првите 25 од Perch или вкупно 50). Значи треба да се направи бинарен класификатор, при што секоја класа освен Bream и Perch се игнорира при тренирањето на множеството.
Притоа земањето на првите 25 примероци да се направи во програмата, а не со рачно копирање!
...
if __name__ == "__main__":
    test_case = input()
    test_case = [float(x) for x in test_case.split(', ')[:-1]] + [test_case.split(', ')[-1]]  # here we already have created test case with whom we will test our trained tree

    (s1, s2) = divide_set(data, -1, 'Bream')  # to divide breams from all other classes i will use the divide_set function s1 will be list of lists where the class is bream and s2 will be with all other classes
    breams = s1[:25]                          # since we need only the first 25 samples we slice the list for 0 to 25
    perches = []                              # now for getting first 25 perch samples we will see another way to do it
    for x in s2:                              # we can iterate all samples in the previosly created s2 set
        if x[-1] == 'Perch':                  # when the class which is on last pos in each list is "Pearch" we will take that sample and store it in another list
            perches.append(x)
            if len(perches) >= 25:            # when the size of the new list gets to 25 we will stop
                break

    training_data = breams + perches          # now we concatenate both lists and we have binary classifier
    tree = build_tree(training_data)          # we train the tree only with Bream and Pearch samples

    predicted_class = classify(test_case, tree)  # because the classify func returns dictionary with key = predicted class and val = number of occurrences of that class in the leaf of the tree
                                                  # the class that we predict will be the class with the biggest value ---> if there are more classes with the same value we usually take the first class by alphabetical order

    # first we will see structured way to take out class from dictionary and in some another task we will see the python way
    maks = 0
    p_class = None
    for key, val in predicted_class.items():     # we iterate over the dict as list of tuples ---? (k, v)
        if val > maks:                           # if the val of that tuple is the current max
            maks = val                           # store that max val for further compares
            p_class = key                        # store that class as the predicted class
        elif val == maks:                        # if that val is the same as the previous max
            if p_class > key:                    # this check means that the value stored in p_class is bigger (alphabetical order) after the value stored in key
                p_class = key                    # then we save that class as the predicted

    print(p_class)          # at the end we just print the predicted class
