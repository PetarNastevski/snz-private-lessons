...
class DecisionNode:
    def __init__(self, col=-1, value=None, results=None, tb=None, fb=None, level=None):   # change 1 ---> add parameter level in the Node constructor
        """
        ...
        self.tb = tb
        self.fb = fb
        self.level = level      # change 2 --- > initialize the level for each node
...
def build_tree(rows, level, scoref=entropy):    # change 3 ---> we add one more argument to the build tree func ---> level
    ...
    if best_gain > 0:
        true_branch = build_tree(best_sets[0], level + 1, scoref)      # change 4 ---> in each recursive call we increment the level for 1 ---> we will start from level 0
        false_branch = build_tree(best_sets[1], level + 1, scoref)
        return DecisionNode(col=best_criteria[0], value=best_criteria[1], level = level,    # change 5 ---> each Node will store one more parametar (level) as we added level to the constructor of the DecisionNode class
...
def print_tree(tree, indent=''):
    ...
    if tree.results:
        ...
        # Се печати условот
        print(str(tree.col) + ':' + str(tree.value) + '? Level=' + str(tree.level))    # change 6 ---> Now we access the level from each Object DecisionNode and print it in the desired format to match the output
