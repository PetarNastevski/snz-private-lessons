...
def get_words_with_ignore(doc):
    ...
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20 and word.lower() not in words_to_ignore:    # the same implementation, we just add to the condition that the words are not in the same list
...
if __name__ == "__main__":
    ...
    classifier1 = NaiveBayes(get_words)                 # first classifier with the standard get_words
    classifier2 = NaiveBayes(get_words_with_ignore)     # second with the changed function

    for red in train_data:
        classifier1.train(red[0], red[1])               # we train both of the classifiers row by row
        classifier2.train(red[0], red[1])

    pred1 = classifier1.classify_document(recenica)     # we predict with both for the given sentence
    pred2 = classifier2.classify_document(recenica)

    print(pred1)           # print the predictions
    print(pred2)

    if pred1 != pred2:             # if the prediction is not the same with both classifiers we print one more line
        print("kontradikcija")
