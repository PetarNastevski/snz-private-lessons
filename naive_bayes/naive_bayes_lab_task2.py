...
if __name__ == "__main__":
    recenica = input()
    classifier = NaiveBayes(get_words)        # create the object NaiveBayes classifier
    for red in train_data:                    # train the classifier row by row using the train function from the class DocumentClassifier
        classifier.train(red[0], red[1])
    words = list(get_words(recenica))         # we need the words that are used in the classification but we need them sorted
    words.sort()                   # because get_words returns set and the elements of a set are unordered we cast that set to list so we can sort the words and get the desired output
    for word in words:             # we iterate the sorted list and calculate the probabilities the word to be in one of two possible classes
        probability1 = classifier.get_feature_per_category_probability(word, "science")  # prob the word is in class science
        weighted_probability1 = classifier.weighted_probability(word, "science", classifier.get_feature_per_category_probability)  #weighted prob for the same
        probability2 = classifier.get_feature_per_category_probability(word, "sport")
        weighted_probability2 = classifier.weighted_probability(word, "sport", classifier.get_feature_per_category_probability)
        print(f'{word} science {round(probability1,5)} {round(weighted_probability1, 5)}')   # print in the desired format
        print(f'{word} sport {round(probability2, 5)} {round(weighted_probability2, 5)}')
    prediction = classifier.classify_document(recenica)                                 # predict for the input
    prob = classifier.get_category_probability_for_document(recenica, prediction)       # calculate the probability which predicts that the given sentence is in the predicted class
    prob = round(log2(prob), 5)    # calculate log2 of that probability
