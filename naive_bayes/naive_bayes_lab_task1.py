...
if __name__ == "__main__":
    recenica = input()
    classifier = NaiveBayes(get_words)                 # here we create an object from the NaiveBayes class and we will use the standard get_words function which returns the important words regarding the classification
    for red in train_data:                            # the difference between training a tree and training naive bayes classifier is that here we need to train row by row
        classifier.train(red[0], red[1])              # so we iterate row by row and we are using train function (first argument is the sentence, second is the given class for that sentence)
    prediction = classifier.classify_document(recenica)  # we use the trained classifier to predict for the given sentence on std input
    print(prediction)   # here the prediction come directly as string, number or whatever the given classes type is and we do not need to take out the class like when working with trees
    probability = classifier.get_category_probability_for_document(recenica, prediction)   # at the end we are calculating the probability that the document on the input is in the predicted class
    print(round(probability, 10))    # printing that prob roundend to 10 decimal places
