'''
Да се најдат петте најслични текстови од тренинг множеството со текстот даден на влез со користење на tf-idf. Притоа,
за наоѓање на сличност да се користи Пирсонова корелација и косинусно растојание. Потребно е да се најдат наслични
документи според намален вокабулар. Намалениот вокабулар се добива со отстранување на 100те зборови што се појавуваат
најчесто во множеството.

Помош: целосниот вокабулар може да го добиете со моменталната имплементација на функцијата get_words, додека за добивање
на намалениот вокабулар потребна е функција со која ќе се игнорираат претходно пресметаните 100 најпојавувани зборови.
Зборовите потребно е да бидат сортирани алфабетски, по што треба да се сортираат според бројот на појавувања.

Излезот треба да има 5 линии со по 2 броеви - првиот е индексот на сличниот текст со Пирсонова корелација, а вториот е
индексот на сличниот текст со косинусно растојание.
'''


from tf_idf_changed_for_exercise5 import *
...
if __name__ == '__main__':
    ...
    rank_cos = rank_documents(text, docs, cosine)
    rank_pear = rank_documents(text, docs, pearson)

    for i in range(5):
        print(f"Rang po slicnost:{i} Cosine-Document #{rank_cos[i][1]} Pearson-Document #{rank_pear[i][1]}")
