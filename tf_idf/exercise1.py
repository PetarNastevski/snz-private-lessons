...
Притоа, за наоѓање на сличност да се користи и Пирсонова корелација и косинусно растојание и да се споредат пронајдените
слични текстови.

Излезот треба да има 4 линии со по 2 броеви - првиот е индексот на сличниот текст со косинусно растојание, вториот е
индексот на сличниот текст со Пеарсонова корелација. Во првата линија се однесува за најсличните текстови, а четвртата
за текстовите кои се 4-ти по сличност со дадениот текст. На крај во петтата линија да се испечати дали пронајдените текстови
според двете метрики се исти или не (True/False).
'''
...
if __name__ == '__main__':
    ...
    #docs = [torka[0] for torka in train_data]  # we can take out the sentences like this
    docs = []                  # or like this with loop
    for tuple in train_data:
        docs.append(tuple[0])

    pear = rank_documents(text, docs, pearson)   # we want the most similar 4 sentences from the list with sentences compared to the sentence given on input
    cos = rank_documents(text, docs, cosine)     # using pearson and cosine

    # we have list of tuples like this [(value, ind_of_doc), (value, ind_of_doc)...] descending order sorted by value
    flag = True
    for i in range(4):        # so the first tuple is the most similar and so on... we need the 4 most similar sentences
        print(f"Rang po slicnost:{i} Cosine-Document #{cos[i][1]} Pearson-Document #{pear[i][1]}")  # print the desired output
        if cos[i][1] != pear[i][1]:   # and check for each line if the two sentences that are most similar are the same or not
            flag = False
    print(f"Dvete metriki davaat ist rezultat:{flag}")   # print if they are the same or not in each line
