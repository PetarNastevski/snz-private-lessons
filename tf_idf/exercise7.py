'''
Дадено е податочно множество (dataset) на спам коментари. Потребно е да се направи класификатор на коментари според
tf-idf векторските репрезентации на коментарите. Овој класификатор ја предвидува класата на моменталниот коментар според
класата која е мнозинство од класите на N-те најслични коментари претставени со tf-idf вектори.

Од стандарден влез се чита бројот на слични коментари N во променливата n, како и коментар кој треба да се класифицира
во spam или ham класа. За да се предвиди класата, најпрво потребно е да се најдат n најслични коментари според косинусно
растојание, по што класата треба да се определи според доминантната класа од множеството на класи добиени од најсличните
коментари. Ако не се пронајдени N најслични документи со сличност поголема од 0, потребно е да се разгледуваат само
добиените најслични документи (чиј што број е помал од N). Доколку бројот на двете класи е еднаков треба да се предвиди класата spam.
'''
...
if __name__ == '__main__':
    ...
    docs = []
    for tuple in dataset:            # we need the documents from the set so we can find the most similar to the input with rank documents
        docs.append(tuple[0])

    result = rank_documents(comment, docs, cosine)   # we got sorted list of tuples where the first tuple contains the index of the sentence that is the most similar
    result = result[:n]                                 # we only need n most similar and because this list is sorted we can just slice the first n elements(tuples)
    finalResult = []
    #rez = [item for item in rez if item[0] > 0]
    for item in result:                                # from the tuples we consider valid the sentences that are there with val > 0
        if item[0] > 0:
            finalResult.append(item)

    n = len(finalResult)                             # now n value may be changed because of the previous filter so we set it to be the lenght od the new list because we will need that to format the output

    counter_spam = 0
    counter_ham = 0

    for el in finalResult:                            # we iterate the rest valid tuples and we check for each index on that position in the given dataset which class that sentence has
        if dataset[el[1]][1] == "spam":        # we will predict the class that is present more
            counter_spam += 1
        else:
            counter_ham += 1

    label = "spam"               # if both classes are counted the same times then we consider spam class as default
    if counter_ham > counter_spam:
        label = "ham"

    if label == "spam":
        print(f"Predvidenata klasa e {label} so {counter_spam}/{n} glasovi.")
    else:
        print(f"Predvidenata klasa e {label} so {counter_ham}/{n} glasovi.")
