'''
За документот на влез, најдете го најсличниот документ од тренинг множеството користејќи косинусно растојание и пирсонов
коефициент на корелација. На излез да се испечатат двата документи (во првата линија документот добиен со косинусно
растојание и во втората линија документот добиен со пирсонов коефициент на корелација). На крај да се испечати „true“
доколку на двата начини се добива истиот документ. Во спротивно да се испечати „false“.
'''
...
if __name__ == '__main__':
    text = input()
    data = []                        # list that will store all sentences (without the class)
    for x in train_data:             # iterate the dataset (list of tuples) and just get the sentence that is on pos 0 in each tuple  ---> we can do this in one line
        data.append(x[0])
    res_cos = rank_documents(text, data, cosine)      # so we rank the docs by cousine and pearson --- same here we have list of tuples as result sorted in descending order first tuple has the index of the document that is the most similar and so on..
    res_pear = rank_documents(text, data, pearson)
    output_pear = data.pop(res_pear[0][1])           # with res_pear[0] we access the first tuple and with [1] we access the second thing in the tuple which is the position of the most similar sentence in the list of sentences
    output_cos = train_data.pop(res_cos[0][1])[0]    # if we pop from train data because train data has tuples we and we need the sentence we have to access [0] from the returned tuple
    # output_cos = output_cos[0]
    print(output_cos)             # print the recommended sentence with both ways ---> cosine and pearson
    print(output_pear)
    if output_cos == output_pear:
        print('True')
    else:
        print('False')