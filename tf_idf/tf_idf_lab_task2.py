'''
Да се изгради дрво на одлука за класификација на текст. Од влез во променливата sample_ind се сместува индексот на
документот чија класа треба да се предвиди. Овој документ се вади од множеството и служи за тестирање, а остатокот е
множество за тренирање.
'''
...
if __name__ == '__main__':
    sample_ind = input()
    sample_ind = int(sample_ind)
    test_data = data.pop(sample_ind)    # take out the tuple with the given index on input
    documents = []                      # we need to seperate sentences and classes
    labels = []
    for x in data:                      # as we can see in the given set labels are on pos 0
        documents.append(x[1])
        labels.append(x[0])

    dataset, df, n, vocab = create_dataset(documents, labels)    # now we create dataset that is compatible for recommending with decision_trees
    tree = build_tree(dataset)                                   # create dataset is made to return those parameters (df, n, vocab)
    test_case = process_document(test_data[1], df, n, vocab)     # because now dataset is converted set from sentences to tf_idf vectors we need to make the test sample vector too so it can be recognized
    pred = max(classify(test_case, tree).items(), key= lambda x: x[1])[0]                # make a prediction for the vector test_case
    print(pred)
